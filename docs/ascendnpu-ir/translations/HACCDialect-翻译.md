...
HACC 获取分片结构体大小函数属性。用于指示设备函数的主机端获取分片结构体大小函数。
...
对于一个原子操作，所有块在 GM 上共享 1 个 `<1xi64>` 类型的 memref，这被称为一个锁，该函数用于推断内核需要多少个锁。
...
在 NPU 设备上，内核函数的调用约定是输出作为输入参数传入。
...
每个设备规格描述单个设备及其硬件属性。每个设备规格可以包含任意数量的可选硬件属性（例如下面的 "UB_SIZE"）。
...
| Parameter | C++ type | Description |
| :-------: | :-------: | ----------- |
| entries | `::llvm::ArrayRef<DataLayoutEntryInterface>` | 单个规格条目 |
