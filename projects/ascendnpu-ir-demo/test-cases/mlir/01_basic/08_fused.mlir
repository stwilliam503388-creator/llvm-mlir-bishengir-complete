// Fused — 算子融合演示
// 操作: C = A + B; D = C * A (连续两个 linalg.generic)
// 一句话: 两步并为一步, 减少中间内存读写
// 专业角色: Kernel Fusion 概念演示, TVM/XLA/Triton 核心优化
// 用在哪: 编译器优化概念演示
...
module {
  func.func @fused(%A: memref<128xf32>, %B: memref<128xf32>, %D: memref<128xf32>) {
    ...
    linalg.generic {indexing_maps = [affine_map<(i) -> (i)>, affine_map<(i) -> (i)>, affine_map<(i) -> (i)>], iterator_types = ["parallel"]} ins(%A, %B : memref<128xf32>, memref<128xf32>) outs(%C : memref<128xf32>) {
    ^bb0(%a: f32, %b: f32, %c: f32):
      %sum = arith.addf %a, %b : f32
      linalg.yield %sum : f32
    }
    linalg.generic {indexing_maps = [affine_map<(i) -> (i)>, affine_map<(i) -> (i)>, affine_map<(i) -> (i)>], iterator_types = ["parallel"]} ins(%C, %A : memref<128xf32>, memref<128xf32>) outs(%D : memref<128xf32>) {
