...
| `linalg.pooling_nhwc_max` | ✅ 已通过 affine.for 替代 | 非可逆 indexing map | 已修复 | 可用 |
| `linalg.pooling_nchw_max` | ✅ 已通过 affine.for 替代 | 非可逆 indexing map | 已修复 | 可用 |
| `linalg.pooling_nchw_sum` | ✅ 已通过 affine.for 替代 | 非可逆 indexing map | 已修复 | 可用 |
...
**但这不意味着不能测试 pooling**——可以用 `affine.for` 手动实现（见下）。

## 已提供的解决方案

使用 **affine.for** 手动实现 pooling 语义。当前所有 3 个 pooling 用例均通过 `mlir-opt` 解析 + LLVM 降级。
...
| linalg.pooling (named) | ✅ affine.for 替代 | ✅ |
| **当前所有 28 个用例** | **✅ 全部通过** | **✅** |
