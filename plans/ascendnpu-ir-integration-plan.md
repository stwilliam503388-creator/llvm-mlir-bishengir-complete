...
| 现有内容 | 缺陷 |
|---------|------|
| README 写着 "AscendNPU-IR 三阶段: Linalg→HFusion→HIVM" | 但没说 **具体哪个文件** 实现了哪个阶段 |
| references/README.md 写着 "核心目录: bishengir/lib/Conversion/" | 但没说 **该目录下每个 Pass 分别做什么** |
| ascendnpu-ir-demo 在跑 `--convert-linalg-to-affine-loops` | 但没说 **这个 pass 在 ascendnpu-ir 源码里对应哪个 .cpp 文件** |
| ascendnpu-ir-op-counter 在写自定义 Pass | 但没说 **如果你想把它注册到 bishengir-opt，应该怎么操作** |
...
ascendnpu-ir-demo 的 test-cases（vecadd/matmul/fused）只是纯 MLIR 文件，没有注释说明：
- 这个 `linalg.generic` 在 ascendnpu-ir 的哪个测试用例里也有？
- 这个 `arith.addf` 在 AscendNPU-IR 里被转换成了什么？
...
| **4. 深度分析笔记索引** | AscendNPU-IR解读 + 文档总结 的位置和内容概要 | 缺陷 2 |
...
|-----------|----------------------|
| `projects/ascendnpu-ir-demo/test-cases/vecadd_128.mlir` | `bishengir/test/Conversion/LinalgToHFusion/linalg-to-hfusion.mlir` |
| `projects/ascendnpu-ir-demo/test-cases/matmul_4x4x4.mlir` | `bishengir/test/Conversion/LinalgToHFusion/matmul-to-hfusion.mlir` |
| `projects/ascendnpu-ir-demo/variants/variant0_baseline.sh` | `bishengir-opt --convert-linalg-to-hfusion` |
| `projects/ascendnpu-ir-demo/variants/variant3_hw_mapping.sh` | `bishengir/lib/Conversion/HFusionToHIVM/hfusion-to-hivm.cpp` |
| `projects/ascendnpu-ir-op-counter/BishengirOpCounter.cpp` | `bishengir/lib/Conversion/LinalgToHFusion/LinalgToHFusion.cpp`（模式） |
| `projects/ascendnpu-ir-op-counter/BishengirPeelTranspose.cpp` | `bishengir/lib/Conversion/ToyCombine.cpp`（模式） |
...
#### C1: ascendnpu-ir-demo test-cases 加头部注释
...
#### C2: ascendnpu-ir-op-counter 源码加文件级注释
...
# 区别: AscendNPU-IR 用 hivm.vadd（向量指令），本 demo 用 llvm.add（标量指令）
```
...
Step 3: 给 ascendnpu-ir-demo test-cases 加头部注释    (3 个 .mlir 文件)
Step 4: 给 ascendnpu-ir-op-counter 加文件级注释       (2 个 .cpp 文件)
...
- ❌ 将 ascendnpu-ir-op-counter 实际注册到 bishengir-opt（需要编译环境）
- ❌ 9 篇翻译文档的审校（已翻译，直接收录）
